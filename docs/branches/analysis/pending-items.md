...
2. If moving toward implementation: `../../architecture/architecture-approach.md`'s modular-monolith recommendation is still proposal-only, not an ADR — accepting it (or choosing otherwise) would be the natural next cross-cutting decision, followed by concrete technology choices.
3. Once this branch's goals are met, promote anything still branch-local here into the permanent knowledge base (it should already be minimal, since decisions have been promoted to ADRs as they were made) and consider this branch's context closed per `../README.md`.

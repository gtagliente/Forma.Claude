...
1. **Workout creation → Exercise existence.** `training-planning-service` just built Workout creation, referencing `Exercise` by `ExerciseId` only, currently unvalidated (`docs/services/training-planning-service/domain.md`).

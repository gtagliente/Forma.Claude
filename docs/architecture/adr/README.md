...
Seven ADRs accepted, all promoted from Iteration 1/2/3 open questions and follow-on architecture decisions once the product owner decided them:
...
Remaining Iteration 1 output is still proposal-only (`../bounded-contexts.md`) except where updated to reflect the seven accepted ADRs above. `../architecture-approach.md` and `../context-map.md` have been updated to reflect ADR-005's outcome; `../context-map.md` also now reflects ADR-007's `identity-service` → `exercise-service`/`training-planning-service` token-validation dependency. `../integration-patterns.md` (previously empty) is now populated with the accepted mechanism underlying ADR-006.
